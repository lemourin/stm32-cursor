...
static void GPIOconfigure(void) {
  ...
  GPIOoutConfigure(GPIO_LCD_CS, LCD_CS_PIN_N, GPIO_OType_PP, GPIO_High_Speed,
                   GPIO_PuPd_NOPULL);

  A0(1); /* Data are sent default. */
  GPIOoutConfigure(GPIO_LCD_A0, LCD_A0_PIN_N, GPIO_OType_PP, GPIO_High_Speed,
                   GPIO_PuPd_NOPULL);

  SDA(0);
  GPIOoutConfigure(GPIO_LCD_SDA, LCD_SDA_PIN_N, GPIO_OType_PP, GPIO_High_Speed,
                   GPIO_PuPd_NOPULL);

  SCK(0); /* Data bit is written on rising clock edge. */
  GPIOoutConfigure(GPIO_LCD_SCK, LCD_SCK_PIN_N, GPIO_OType_PP, GPIO_High_Speed,
                   GPIO_PuPd_NOPULL);
}
...
void LCDconfigure() {
  ...
  LCDsetColors(LCD_COLOR_GREEN, LCD_COLOR_RED);
  /* Initialize hardware. */
  GPIOconfigure();
...
void LCDputchar(char c) {
  if (c == '\n')
    ...
    if (c >= FIRST_CHAR && c <= LAST_CHAR && Line >= 0 && Line < TextHeight &&
...
uint16_t screen[LCD_PIXEL_WIDTH][LCD_PIXEL_HEIGHT];
...
void clamp_rectangle(int *x, int *y, int *size_x, int *size_y) {
  if (*x < 0) {
    *size_x += *x;
    *x = 0;
  }
  ...
  *size_x = clamp(*size_x, 0, LCD_PIXEL_WIDTH - *x);
  *size_y = clamp(*size_y, 0, LCD_PIXEL_HEIGHT - *y);
}

bool is_on_screen(int x, int y) {
  return x >= 0 && x < LCD_PIXEL_WIDTH && y >= 0 && y < LCD_PIXEL_HEIGHT;
}

void LCDdrawSquare(int x, int y, int size_x, int size_y) {
  CS(0);
  clamp_rectangle(&x, &y, &size_x, &size_y);
  LCDsetRectangle(x, y, x + size_x - 1, y + size_y - 1);
  for (int i = 0; i < size_y; i++) {
    for (int j = 0; j < size_x; j++) {
      LCDwriteData16(screen[x + j][y + i]);
    }
  }
  CS(1);
}

void lcd_draw_square(int x, int y, int size_x, int size_y, uint16_t color) {
  for (int i = 0; i < size_x; i++)
    for (int j = 0; j < size_y; j++) {
      if (is_on_screen(x + i, y + j)) {
        if (size_x == CURSOR_SIZE && size_y == CURSOR_SIZE) {
          if (cursor[i][j] == '#') screen[x + i][y + j] = color;
        } else
...
void initialize_lcd() {
  ...
  lcd.pos_x = LCD_PIXEL_WIDTH / 2 - CURSOR_SIZE / 2;
  lcd.pos_y = LCD_PIXEL_HEIGHT / 2 - CURSOR_SIZE / 2;

  lcd_draw_square(0, 0, LCD_PIXEL_WIDTH, LCD_PIXEL_HEIGHT, BackColor);
  lcd_draw_square(lcd.pos_x, lcd.pos_y, CURSOR_SIZE, CURSOR_SIZE, TextColor);
  LCDdrawSquare(0, 0, LCD_PIXEL_WIDTH, LCD_PIXEL_HEIGHT);
}

void lcd_move(int dx, int dy) {
  lcd.pos_x = clamp(lcd.pos_x + dx, -CURSOR_SIZE / 2,
                    LCD_PIXEL_WIDTH - CURSOR_SIZE / 2);
  lcd.pos_y = clamp(lcd.pos_y + dy, -CURSOR_SIZE / 2,
                    LCD_PIXEL_HEIGHT - CURSOR_SIZE / 2);
}

void lcd_update(int *current_x, int *current_y) {
  if (*current_x != lcd.pos_x || *current_y != lcd.pos_y) {
    int prev_x = *current_x, prev_y = *current_y;
    int lcd_x = lcd.pos_x, lcd_y = lcd.pos_y;
    if (abs(lcd_x - prev_x) <= CURSOR_SIZE ||
        abs(lcd_y - prev_y) <= CURSOR_SIZE) {
      lcd_draw_square(prev_x, prev_y, CURSOR_SIZE, CURSOR_SIZE, BackColor);
      lcd_draw_square(lcd_x, lcd_y, CURSOR_SIZE, CURSOR_SIZE, TextColor);

      int x = prev_x < lcd_x ? prev_x : lcd_x;
      int y = prev_y < lcd_y ? prev_y : lcd_y;
      LCDdrawSquare(x, y, CURSOR_SIZE + abs(lcd_x - prev_x),
                    CURSOR_SIZE + abs(lcd_y - prev_y));
    } else {
      lcd_draw_square(prev_x, prev_y, CURSOR_SIZE, CURSOR_SIZE, BackColor);
      LCDdrawSquare(prev_x, prev_y, CURSOR_SIZE, CURSOR_SIZE);
      lcd_draw_square(lcd_x, lcd_y, CURSOR_SIZE, CURSOR_SIZE, TextColor);
      LCDdrawSquare(lcd_x, lcd_y, CURSOR_SIZE, CURSOR_SIZE);
    }
    *current_x = lcd_x;
    *current_y = lcd_y;
  }
